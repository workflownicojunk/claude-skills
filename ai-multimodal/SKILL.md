...
**WICHTIG (Desktop/Lightness Fitness):** In `~/Desktop/.env` heißt der Key `GOOGLE_API_KEY`, nicht `GEMINI_API_KEY`. Korrekte Übergabe:
```bash
GEMINI_API_KEY=$(grep ^GOOGLE_API_KEY ~/Desktop/.env | cut -d'=' -f2-) python3 scripts/gemini_batch_process.py ...
```
`source .env && export GEMINI_API_KEY` funktioniert NICHT in Background-Tasks (neue Shell erbt keine Variablen).
